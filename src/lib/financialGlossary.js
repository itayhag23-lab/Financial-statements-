...
const FINANCIAL_GLOSSARY = {
  ...
  cogs: {
    ...
    why: "If this creeps too close to revenue, every sale barely covers itself, and you can be busy and still broke. Keeping it in check is what makes growth actually profitable instead of just bigger.",
  },
  'cogs-direct': {
    term: 'Direct Costs',
  ...
  'opex-other': {
    ...
    why: "Worth a glance if it grows large, since that usually means something in here deserves its own line so you can actually manage it.",
  },
  'op-inc': {
  ...
  'net-inc': {
    ...
    why: "This is the headline number everyone uses to ask 'did the business make money?' But check Operating Income too, since a single one-off item can make Net Income misleading on its own.",
  },
  ...
  'other-ca': {
    ...
    why: "Usually minor, but worth a glance if it's growing, since that can mean cash is getting tied up somewhere unexpected.",
  },
  ppe: {
  ...
  'cf-ar': {
    ...
    why: "If customers owe you more this period than last, that's revenue you've recognized but haven't collected, and it reduces your actual cash even if profit looks fine.",
...
export function getGlossaryEntry(key, parentId) {
  return (key && FINANCIAL_GLOSSARY[key]) || (parentId && CATEGORY_FALLBACK[parentId]) || null;
}
