    ...
    <div className="koala-page" style={{ background: P.bg, minHeight: '100vh', ...body, color: P.ink, overflowX: 'hidden' }}>
      ...
      <section id="how" style={{ background: P.bg, borderBottom: `1px solid ${P.border}` }}>
        <div style={{ ...maxW, padding: `${vp} ${sp}` }}>
          ...
          <Reveal delay={100} style={{ display: 'flex', flexDirection: mob ? 'column' : 'row', alignItems: 'stretch', gap: mob ? 0 : 0 }}>
            {[
              { n: '01', title: 'Describe your business', body: 'Type one sentence: "B2B SaaS, $30K MRR, targeting $5M ARR, 18-month runway." Koala reads your sector, stage, and scale automatically.',
                visual: (
                  <div style={{ background: P.bg, border: `1px solid ${P.border}`, borderRadius: 8, padding: '9px 11px', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ ...mono, fontSize: 10.5, color: P.ink2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>B2B SaaS, $30K MRR, targeting $5M ARR...</span>
                    <span style={{ width: 1, height: 12, background: P.accent, animation: 'caretBlink 1s step-end infinite', flexShrink: 0 }} />
                  </div>
                ) },
              { n: '02', title: 'Model builds itself', body: 'A fully-linked Income Statement, Cash Flow, and Balance Sheet — seeded with sector-calibrated assumptions and three live scenarios — in under 60 seconds.',
                visual: (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {['Income Statement', 'Cash Flow', 'Balance Sheet'].map((s) => (
                      <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 6, ...body, fontSize: 11, color: P.ink2 }}><Check size={12} color={P.accent} style={{ flexShrink: 0 }} />{s}</div>
                    ))}
                  </div>
                ) },
              { n: '03', title: 'Refine, share, raise', body: 'Adjust any assumption, run what-if scenarios with the AI advisor, then share a live interactive report directly with your investors or board.',
                visual: (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: P.bg, border: `1px solid ${P.border}`, borderRadius: 8, padding: '8px 11px' }}>
                    <Link2 size={12} color={P.muted} style={{ flexShrink: 0 }} />
                    <span style={{ ...mono, fontSize: 10, color: P.ink2, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>koala.app/r/acme</span>
                    <span style={{ ...body, fontSize: 9, fontWeight: 700, color: P.accentText, background: P.accentSoft, borderRadius: 10, padding: '2px 7px', flexShrink: 0 }}>LIVE</span>
                  </div>
                ) },
            ].map(({ n, title, visual, body: text }, i) => (
              <React.Fragment key={n}>
                <div style={{ padding: mob ? '20px 16px' : '26px 22px', background: P.bgAlt, borderRadius: 12, border: `1px solid ${P.border}`, flex: 1, minWidth: 0, position: 'relative' }}>
                  <div style={{ ...disp, fontSize: 32, fontWeight: 800, color: P.border, position: 'absolute', top: 14, right: 18, lineHeight: 1, userSelect: 'none' }}>{n}</div>
                  <div style={{ marginBottom: 16, marginRight: 36 }}>{visual}</div>
                  <h3 style={{ ...disp, fontSize: mob ? 17 : 19, fontWeight: 700, color: P.ink, marginBottom: 8, letterSpacing: '-0.01em' }}>{title}</h3>
                  <p style={{ ...body, fontSize: mob ? 13 : 13.5, lineHeight: 1.65, color: P.ink2, margin: 0 }}>{text}</p>
                </div>
                {i < 2 && (
                  mob
                    ? <div style={{ display: 'flex', justifyContent: 'center', padding: '6px 0', color: P.faint }}><ArrowRight size={16} style={{ transform: 'rotate(90deg)' }} /></div>
                    : <div style={{ display: 'flex', alignItems: 'center', padding: '0 12px', color: P.faint, flexShrink: 0 }}><ArrowRight size={18} /></div>
                )}
              </React.Fragment>
            ))}
          </Reveal>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: tab ? '1fr' : '1fr 1fr', gap: tab ? 32 : 64, alignItems: 'center' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {[
                  { label: 'SOC 2 Type II',             desc: 'Annual third-party audit of our security controls' },
                  { label: '256-bit AES Encryption',     desc: 'At rest and in transit, on every plan' },
                  { label: 'GDPR & CCPA Compliant',      desc: 'Full data residency and deletion on request' },
                  { label: 'Role-Based Access Control',  desc: 'Granular permissions for team and board sharing' },
                ].map(({ label, desc }, i) => (
                  <div key={label} style={{ padding: '13px 0', borderTop: i ? `1px solid ${P.borderDark}` : 'none' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: mob ? 'wrap' : 'nowrap' }}>
                      <Check size={14} color={P.accent} style={{ flexShrink: 0 }} />
                      <span style={{ ...body, fontSize: 14, fontWeight: 600, color: '#F1F5F9', flexShrink: 0, whiteSpace: 'nowrap' }}>{label}</span>
                      {!mob && <span style={{ flex: 1, borderBottom: `1px dotted ${P.borderDark}`, marginBottom: 3, minWidth: 16 }} />}
                      {!mob && <span style={{ ...body, fontSize: 12.5, color: '#94A3B8', textAlign: 'right', flexShrink: 0, maxWidth: 230 }}>{desc}</span>}
                    </div>
                    {mob && <div style={{ ...body, fontSize: 12.5, color: '#CBD5E1', marginTop: 6, marginLeft: 24 }}>{desc}</div>}
                  </div>
                ))}
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {['SOC 2', 'GDPR', 'ISO 27001', 'CCPA'].map((badge) => (
                  <div key={badge} style={{ background: P.bgDarkAlt, border: `1px dashed rgba(16,185,129,0.4)`, borderRadius: 8, padding: '16px 14px', textAlign: 'center', position: 'relative' }}>
                    <span style={{ ...mono, fontSize: 10, fontWeight: 700, color: P.accent, position: 'absolute', top: 9, right: 11 }}>✓</span>
                    <div style={{ ...mono, fontSize: 13.5, fontWeight: 700, color: '#F1F5F9', letterSpacing: '0.01em' }}>{badge}</div>
                    <div style={{ ...body, fontSize: 9.5, color: mob ? '#CBD5E1' : '#94A3B8', marginTop: 4, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Verified</div>
                  </div>
                ))}
              </div>
