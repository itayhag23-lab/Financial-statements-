...
import{Plus,Trash2,X,ChevronDown,ChevronRight,TrendingUp,TrendingDown,AlertTriangle,Download,Save,Edit3,Percent,Sliders,Check,Info,Target,BarChart3,Sparkles,RefreshCw,Upload,FileSpreadsheet,FileText,Calculator,ArrowRight,ArrowLeft,Search,Wand2,Loader2}from 'lucide-react';
...
bakery:{label:'Bakery / Patisserie',icon:'🥐',category:'Food & Beverage',blurb:'Baked goods, pastries, café-bakery.',benchmarks:{grossMargin:[62,72],opMargin:[5,12],netMargin:[4,9]},seedBase:{revenue:450,revGrowth:8,cogsPct:30,opexBase:240,opexGrowth:5,taxPct:5},extraRows:[{parentId:'opex',label:'Salaries & Wages',defaultMode:'flatGrowth',baseValue:130,flatRate:6},{parentId:'opex',label:'Rent & Utilities',defaultMode:'flatGrowth',baseValue:70,flatRate:4}],peers:[{name:'Panera Bread',netMargin:8},{name:'Local bakery',netMargin:5}]},
bar:{label:'Bar / Pub',icon:'🍺',category:'Food & Beverage',blurb:'Bar, pub, or nightlife venue.',benchmarks:{grossMargin:[68,80],opMargin:[8,18],netMargin:[6,14]},seedBase:{revenue:900,revGrowth:7,cogsPct:26,opexBase:480,opexGrowth:5,taxPct:5},extraRows:[{parentId:'opex',label:'Salaries & Wages',defaultMode:'flatGrowth',baseValue:250,flatRate:6},{parentId:'opex',label:'Rent & Licenses',defaultMode:'flatGrowth',baseValue:120,flatRate:4}],peers:[{name:'National pub chain',netMargin:9},{name:'Neighborhood bar',netMargin:12}]},
subscriptionbox:{label:'Subscription Box',icon:'📦',category:'Retail',blurb:'Recurring curated product boxes.',benchmarks:{grossMargin:[40,55],opMargin:[2,10],netMargin:[1,7]},seedBase:{revenue:600,revGrowth:30,cogsPct:38,opexBase:220,opexGrowth:22,taxPct:4},extraRows:[{parentId:'cogs',label:'Shipping & Fulfillment',defaultMode:'percentOfRevenue',pctOfRev:10},{parentId:'opex',label:'Customer Acquisition (CAC)',defaultMode:'percentOfRevenue',pctOfRev:15}],peers:[{name:'BARK (BarkBox)',netMargin:-5},{name:'Profitable niche box',netMargin:7}]},
wholesale:{label:'Wholesale / Distribution',icon:'🏷️',category:'Retail',blurb:'Buy in bulk, sell to businesses.',benchmarks:{grossMargin:[15,28],opMargin:[3,8],netMargin:[2,6]},seedBase:{revenue:3000,revGrowth:10,cogsPct:78,opexBase:320,opexGrowth:8,taxPct:5},extraRows:[{parentId:'opex',label:'Warehouse & Logistics',defaultMode:'flatGrowth',baseValue:140,flatRate:6}],peers:[{name:'Sysco',netMargin:3},{name:'Regional distributor',netMargin:5}]},
salon:{label:'Salon / Barbershop',icon:'💇',category:'Services',blurb:'Hair, beauty, or grooming studio.',benchmarks:{grossMargin:[80,90],opMargin:[8,20],netMargin:[6,16]},seedBase:{revenue:400,revGrowth:10,cogsPct:12,opexBase:220,opexGrowth:6,taxPct:5},extraRows:[{parentId:'opex',label:'Stylist Commissions',defaultMode:'percentOfRevenue',pctOfRev:35},{parentId:'opex',label:'Rent & Utilities',defaultMode:'flatGrowth',baseValue:90,flatRate:4}],peers:[{name:'Regis Corp',netMargin:4},{name:'Independent salon',netMargin:13}]},
autorepair:{label:'Auto Repair Shop',icon:'🔧',category:'Services',blurb:'Vehicle maintenance and repair.',benchmarks:{grossMargin:[42,58],opMargin:[10,22],netMargin:[7,16]},seedBase:{revenue:700,revGrowth:7,cogsPct:18,opexBase:260,opexGrowth:5,taxPct:5},extraRows:[{parentId:'cogs',label:'Parts & Materials',defaultMode:'percentOfRevenue',pctOfRev:25},{parentId:'opex',label:'Rent & Equipment',defaultMode:'flatGrowth',baseValue:110,flatRate:4}],peers:[{name:'Monro (national)',netMargin:6},{name:'Independent shop',netMargin:12}]},
cleaning:{label:'Cleaning Service',icon:'🧼',category:'Services',blurb:'Residential or commercial cleaning.',benchmarks:{grossMargin:[42,58],opMargin:[12,25],netMargin:[8,18]},seedBase:{revenue:350,revGrowth:18,cogsPct:37,opexBase:90,opexGrowth:12,taxPct:5},extraRows:[{parentId:'cogs',label:'Supplies & Equipment',defaultMode:'percentOfRevenue',pctOfRev:8},{parentId:'opex',label:'Admin & Scheduling',defaultMode:'flatGrowth',baseValue:50,flatRate:10}],peers:[{name:'ABM Industries',netMargin:4},{name:'Residential cleaning co',netMargin:15}]},
realestate:{label:'Real Estate Brokerage',icon:'🏠',category:'Services',blurb:'Property sales and agent commissions.',benchmarks:{grossMargin:[42,62],opMargin:[10,25],netMargin:[8,20]},seedBase:{revenue:600,revGrowth:12,cogsPct:50,opexBase:130,opexGrowth:10,taxPct:8},extraRows:[{parentId:'opex',label:'Marketing & Listings',defaultMode:'flatGrowth',baseValue:70,flatRate:8}],peers:[{name:'Anywhere (Realogy)',netMargin:4},{name:'Boutique brokerage',netMargin:16}]},
dental:{label:'Medical / Dental Clinic',icon:'🦷',category:'Services',blurb:'Private clinic or dental practice.',benchmarks:{grossMargin:[60,75],opMargin:[12,25],netMargin:[10,20]},seedBase:{revenue:900,revGrowth:8,cogsPct:30,opexBase:420,opexGrowth:5,taxPct:8},extraRows:[{parentId:'opex',label:'Clinical Salaries',defaultMode:'flatGrowth',baseValue:230,flatRate:6},{parentId:'opex',label:'Equipment & Rent',defaultMode:'flatGrowth',baseValue:120,flatRate:4}],peers:[{name:'Dental service org (DSO)',netMargin:8},{name:'Private practice',netMargin:18}]},
events:{label:'Events / Catering',icon:'🎉',category:'Services',blurb:'Catering, planning, or event services.',benchmarks:{grossMargin:[38,55],opMargin:[8,18],netMargin:[5,14]},seedBase:{revenue:500,revGrowth:15,cogsPct:35,opexBase:130,opexGrowth:10,taxPct:6},extraRows:[{parentId:'cogs',label:'Staffing & Rentals',defaultMode:'percentOfRevenue',pctOfRev:15},{parentId:'opex',label:'Marketing & Sales',defaultMode:'flatGrowth',baseValue:60,flatRate:8}],peers:[{name:'Large caterer',netMargin:7},{name:'Independent planner',netMargin:14}]},
hotel:{label:'Hotel / B&B / Rental',icon:'🏨',category:'Services',blurb:'Hotel, B&B, or short-term rentals.',benchmarks:{grossMargin:[58,75],opMargin:[10,25],netMargin:[5,18]},seedBase:{revenue:1200,revGrowth:9,cogsPct:28,opexBase:560,opexGrowth:6,taxPct:6},extraRows:[{parentId:'opex',label:'Property & Maintenance',defaultMode:'flatGrowth',baseValue:260,flatRate:5},{parentId:'opex',label:'Staff Salaries',defaultMode:'flatGrowth',baseValue:220,flatRate:5}],peers:[{name:'Marriott',netMargin:9},{name:'Boutique B&B',netMargin:12}]},
logistics:{label:'Logistics / Delivery',icon:'🚛',category:'Services',blurb:'Trucking, delivery, or freight.',benchmarks:{grossMargin:[22,38],opMargin:[5,12],netMargin:[3,9]},seedBase:{revenue:1400,revGrowth:12,cogsPct:40,opexBase:240,opexGrowth:8,taxPct:5},extraRows:[{parentId:'cogs',label:'Fuel & Vehicle',defaultMode:'percentOfRevenue',pctOfRev:30},{parentId:'opex',label:'Insurance & Permits',defaultMode:'flatGrowth',baseValue:90,flatRate:5}],peers:[{name:'FedEx',netMargin:5},{name:'Regional carrier',netMargin:8}]},
marketplace:{label:'Online Marketplace',icon:'🔁',category:'Digital',blurb:'Two-sided platform, take-rate model.',benchmarks:{grossMargin:[55,75],opMargin:[-10,15],netMargin:[-5,12]},seedBase:{revenue:500,revGrowth:45,cogsPct:22,opexBase:360,opexGrowth:30,taxPct:4},extraRows:[{parentId:'cogs',label:'Payment Processing',defaultMode:'percentOfRevenue',pctOfRev:8},{parentId:'opex',label:'Customer Acquisition (CAC)',defaultMode:'percentOfRevenue',pctOfRev:25}],peers:[{name:'Etsy',netMargin:16},{name:'Early-stage marketplace',netMargin:-8}]},
elearning:{label:'Online Courses / EdTech',icon:'🎓',category:'Digital',blurb:'Courses, cohorts, or learning platform.',benchmarks:{grossMargin:[65,85],opMargin:[10,30],netMargin:[8,25]},seedBase:{revenue:350,revGrowth:40,cogsPct:12,opexBase:150,opexGrowth:25,taxPct:5},extraRows:[{parentId:'cogs',label:'Hosting & Content',defaultMode:'percentOfRevenue',pctOfRev:10},{parentId:'opex',label:'Marketing & Sales',defaultMode:'percentOfRevenue',pctOfRev:25}],peers:[{name:'Coursera',netMargin:-6},{name:'Profitable course creator',netMargin:30}]},
construction:{label:'Construction / Contractor',icon:'🏗️',category:'Other',blurb:'Building, trades, or contracting.',benchmarks:{grossMargin:[18,32],opMargin:[4,12],netMargin:[3,9]},seedBase:{revenue:2000,revGrowth:10,cogsPct:55,opexBase:300,opexGrowth:8,taxPct:6},extraRows:[{parentId:'cogs',label:'Materials & Subcontractors',defaultMode:'percentOfRevenue',pctOfRev:18},{parentId:'opex',label:'Equipment & Insurance',defaultMode:'flatGrowth',baseValue:120,flatRate:5}],peers:[{name:'D.R. Horton (large)',netMargin:10},{name:'Local contractor',netMargin:6}]},
...
bakery:['Prime cost (ingredients + labor) runs 50-60% of revenue. Flour and butter prices swing.','Unsold fresh product is pure loss; waste above 5-8% quietly erases the margin.'],
bar:['Pour cost (beverage COGS) above 25% usually means over-pouring, comps, or shrinkage.','Revenue concentrates in Thu-Sat nights. One slow weekend dents the whole month.'],
subscriptionbox:['Monthly churn of 5-10% is normal. Model it or you will overstate steady-state revenue.','Shipping + fulfillment eats 10-15% of revenue; lock in carrier rates early.'],
wholesale:['Net margins are thin (2-6%). A few points of COGS inflation can wipe out profit.','Inventory and 30-60 day receivables tie up cash; model working capital, not just P&L.'],
salon:['Stylist commissions or booth rent (35-50% of service revenue) is the dominant cost.','Chair utilization below 60% means you are paying for idle capacity.'],
autorepair:['Labor hours are far more profitable than parts. Watch the parts/labor mix.','Technician productivity (billed vs. paid hours) of 75-85% drives the bottom line.'],
cleaning:['Labor is 40-50% of revenue and turnover is high. Recruiting is an ongoing cost.','Recurring contracts beat one-offs; without them revenue is lumpy and you re-pay CAC.'],
realestate:['Most revenue is paid out as agent commission splits (50-70%), so net is thin.','Transaction volume is rate-sensitive; a slow market can halve revenue quickly.'],
dental:['Insurance write-offs reduce collected revenue 10-25% below billed amounts.','Chair/operatory utilization and hygienist productivity drive most of the profit.'],
events:['Revenue is seasonal and project-based. Model the gaps between bookings.','Deposits arrive before costs; track cash timing or a busy season can still go negative.'],
hotel:['RevPAR (occupancy × rate) is the real driver. Model occupancy, not just nightly rate.','High fixed costs push break-even occupancy to 55-65%; below that you lose money.'],
logistics:['Fuel is 25-35% of cost and volatile. A fuel surcharge clause protects margin.','Empty (deadhead) miles and the driver shortage erode an already thin margin.'],
marketplace:['Take rate (5-20%) on GMV is your real revenue, not the gross transaction value.','Both sides must grow together; liquidity, not signups, is the metric that matters.'],
elearning:['Refunds and low completion reduce realized revenue. Model refunds, not just sales.','Paid acquisition can exceed lifetime value fast; organic/content is the durable channel.'],
construction:['Fixed-bid jobs carry overrun risk. A 10% materials spike can erase the profit.','Progress billing vs. cost timing creates cash gaps; retainage holds 5-10% until completion.'],
...
if(!open)return null;
// Derive labels straight from the sector bank so new sectors never drift out of sync.
const BB_LABELS=Object.fromEntries(Object.entries(BB).map(([k,b])=>[k,b.label]));
// Local emerald palette aligned to the landing page (AA-safe deep emerald on white).
const AD='#047857',AM='#059669',AB='#10B981',ASOFT='#ECFDF5',MINT='#EFFBF5',BORDER='#E5E7EB',BFAINT='#EEF0F3';
const sectorIcon=result?BB[result.sectorKey]?.icon:null;
const genReady=desc.trim()&&status!=='generating';
const focusRing=e=>{e.target.style.borderColor=AD;e.target.style.boxShadow=`0 0 0 3px ${ASOFT}`;};
const blurRing=e=>{e.target.style.borderColor=BORDER;e.target.style.boxShadow='none';};
const primaryEnter=(e,on)=>{if(!on)return;e.currentTarget.style.background=AM;e.currentTarget.style.transform='translateY(-1px)';e.currentTarget.style.boxShadow='0 10px 24px -8px rgba(4,120,87,0.45)';};
const primaryLeave=(e,on)=>{e.currentTarget.style.background=on?AD:'#CBD5E1';e.currentTarget.style.transform='translateY(0)';e.currentTarget.style.boxShadow=on?'0 1px 2px rgba(15,23,42,0.08)':'none';};
return(
React.createElement(React.Fragment,null,
React.createElement('style',null,'@keyframes koala-spin{to{transform:rotate(360deg)}}'),
React.createElement('div',{style:{position:'fixed',inset:0,zIndex:51,display:'flex',alignItems:'center',justifyContent:'center',padding:16,background:'rgba(15,23,42,0.55)',backdropFilter:'blur(4px)'},onClick:onClose},
React.createElement('div',{className:'anim-fade-in',style:{background:C.surface,border:`1px solid ${BFAINT}`,borderRadius:20,boxShadow:'0 40px 90px -28px rgba(15,23,42,0.45), 0 12px 32px -16px rgba(15,23,42,0.18)',width:'min(560px,100%)',maxHeight:'calc(100dvh - 32px)',display:'flex',flexDirection:'column',overflow:'hidden'},onClick:e=>e.stopPropagation()},
// Header
React.createElement('div',{style:{flexShrink:0,padding:'20px 24px',borderBottom:`1px solid ${BFAINT}`,background:C.surface,display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12}},
React.createElement('div',{style:{display:'flex',alignItems:'center',gap:13,minWidth:0}},
React.createElement('div',{style:{width:44,height:44,borderRadius:12,background:ASOFT,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}},React.createElement(Sparkles,{size:21,color:AD})),
React.createElement('div',{style:{minWidth:0}},
React.createElement('div',{className:'ff-body',style:{color:AD,fontSize:11,fontWeight:700,letterSpacing:'0.14em',textTransform:'uppercase'}},'AI Model Builder'),
React.createElement('h3',{className:'ff-display',style:{fontSize:22,fontWeight:700,color:C.ink,marginTop:3,letterSpacing:'-0.02em',lineHeight:1.1}},'Build from description')
)
),
React.createElement('button',{onClick:onClose,'aria-label':'Close',style:{background:'transparent',border:'none',cursor:'pointer',color:C.faint,padding:6,borderRadius:8,display:'flex',flexShrink:0}},React.createElement(X,{size:18}))
),
// Body
React.createElement('div',{style:{padding:'22px 24px',flex:'1 1 auto',minHeight:0,overflowY:'auto',WebkitOverflowScrolling:'touch'}},
status==='generating'
?React.createElement('div',{style:{padding:'34px 20px',display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center'}},
React.createElement('div',{style:{width:54,height:54,borderRadius:15,background:ASOFT,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:18}},React.createElement(Loader2,{size:25,color:AD,style:{animation:'koala-spin 0.9s linear infinite'}})),
React.createElement('div',{className:'ff-display',style:{fontSize:18,fontWeight:700,color:C.ink,letterSpacing:'-0.01em'}},'Building your model…'),
React.createElement('div',{className:'ff-body',style:{fontSize:13,color:C.muted,marginTop:7,lineHeight:1.6,maxWidth:320}},'Reading your description, selecting a sector, and calibrating realistic assumptions.')
)
:status!=='success'
?React.createElement('div',null,
React.createElement('div',{className:'ff-body',style:{fontSize:14,color:C.ink2,marginBottom:14,lineHeight:1.6}},'Describe your business in plain English. The AI selects a sector, sets realistic assumptions, and seeds a complete, fully-linked model.'),
React.createElement('textarea',{ref:taRef,value:desc,onChange:e=>setDesc(e.target.value),onFocus:focusRing,onBlur:blurRing,placeholder:'e.g. "A SaaS startup in Israel with ~$30K MRR targeting B2B clients, 3 employees, pre-Series A"',rows:5,className:'ff-body',style:{width:'100%',padding:'14px 15px',borderRadius:12,border:`1px solid ${BORDER}`,background:C.surface,color:C.ink,fontSize:14,lineHeight:1.6,resize:'vertical',outline:'none',boxSizing:'border-box',transition:'border-color 160ms, box-shadow 160ms'}}),
React.createElement('div',{className:'ff-body',style:{fontSize:12,color:C.faint,marginTop:10,display:'flex',alignItems:'center',gap:7,lineHeight:1.5}},React.createElement(Info,{size:13,color:C.faint,style:{flexShrink:0}}),'Include sector, revenue, team size, and stage for the closest fit.'),
status==='error'&&React.createElement('div',{style:{marginTop:14,padding:'11px 13px',borderRadius:10,background:C.rustSoft,border:`1px solid ${C.rust}22`,fontSize:12.5,color:C.rust,fontFamily:'Inter,system-ui,sans-serif',lineHeight:1.5}},err)
)
:React.createElement('div',{className:'anim-fade-in'},
React.createElement('div',{style:{padding:'16px 18px',borderRadius:14,background:MINT,border:`1px solid ${AB}33`,marginBottom:14,display:'flex',gap:13,alignItems:'flex-start'}},
React.createElement('div',{style:{width:42,height:42,borderRadius:11,background:C.surface,border:`1px solid ${AB}33`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:21,lineHeight:1,flexShrink:0}},sectorIcon||React.createElement(Check,{size:19,color:AD})),
React.createElement('div',{style:{flex:1,minWidth:0}},
React.createElement('div',{className:'ff-body',style:{fontSize:11,fontWeight:700,color:AD,letterSpacing:'0.12em',textTransform:'uppercase',display:'flex',alignItems:'center',gap:6}},React.createElement(Check,{size:12,color:AD}),'Sector detected'),
React.createElement('div',{className:'ff-display',style:{fontSize:18,fontWeight:700,color:C.ink,marginTop:3,letterSpacing:'-0.01em'}},BB_LABELS[result.sectorKey]),
React.createElement('div',{className:'ff-body',style:{fontSize:13,color:C.ink2,lineHeight:1.6,marginTop:6}},(result.rationale||'Sector-calibrated assumptions seeded.'))
)
),
result.overrides&&result.overrides.length>0&&React.createElement('div',{style:{background:C.surface,border:`1px solid ${BFAINT}`,borderRadius:12,padding:'14px 16px'}},
React.createElement('div',{className:'ff-body',style:{fontSize:10,fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase',color:C.muted,marginBottom:11}},`${result.overrides.length} override${result.overrides.length!==1?'s':''} applied`),
React.createElement('div',{style:{display:'flex',flexDirection:'column',gap:9}},
result.overrides.slice(0,5).map(ov=>React.createElement('div',{key:ov.rowId,style:{display:'flex',alignItems:'baseline',gap:9}},
React.createElement('span',{style:{width:6,height:6,borderRadius:'50%',background:AB,flexShrink:0,alignSelf:'center'}}),
React.createElement('span',{className:'ff-body',style:{fontSize:12.5,fontWeight:600,color:C.ink,flexShrink:0}},ov.rowId),
React.createElement('span',{className:'ff-body',style:{fontSize:12.5,color:C.muted}},
...
// Footer
React.createElement('div',{style:{flexShrink:0,padding:'16px 24px',borderTop:`1px solid ${BFAINT}`,background:C.surface,display:'flex',alignItems:'center',justifyContent:'space-between',gap:10}},
React.createElement('button',{onClick:status==='success'?onClose:(onUseWizard||onClose),className:'ff-body',style:{fontSize:13,fontWeight:500,color:C.muted,background:'transparent',border:'none',cursor:'pointer',padding:'8px 6px'}},status==='success'?'Cancel':'Use the guided wizard instead'),
status==='success'
?React.createElement('button',{onClick:apply,className:'ff-body',onMouseEnter:e=>primaryEnter(e,true),onMouseLeave:e=>primaryLeave(e,true),style:{fontSize:14,fontWeight:600,color:'#fff',background:AD,border:'none',cursor:'pointer',padding:'11px 22px',borderRadius:999,display:'inline-flex',alignItems:'center',gap:8,boxShadow:'0 1px 2px rgba(15,23,42,0.08)',transition:'background 160ms, box-shadow 160ms, transform 160ms'}},React.createElement(Check,{size:16}),'Apply this model')
:React.createElement('button',{onClick:generate,disabled:!genReady,className:'ff-body',onMouseEnter:e=>primaryEnter(e,genReady),onMouseLeave:e=>primaryLeave(e,genReady),style:{fontSize:14,fontWeight:600,color:'#fff',background:genReady?AD:'#CBD5E1',border:'none',cursor:genReady?'pointer':'not-allowed',padding:'11px 22px',borderRadius:999,display:'inline-flex',alignItems:'center',gap:8,boxShadow:genReady?'0 1px 2px rgba(15,23,42,0.08)':'none',transition:'background 160ms, box-shadow 160ms, transform 160ms'}},status==='generating'?'Generating…':'Generate model',status!=='generating'&&React.createElement(ArrowRight,{size:16}))
)
)
)
)
...
const focusRing=e=>{e.target.style.borderColor=AD;e.target.style.boxShadow=`0 0 0 3px ${ASOFT}`;};
const blurRing=e=>{e.target.style.borderColor=BORDER;e.target.style.boxShadow='none';};
const nextEnter=e=>{if(!canAdv)return;e.currentTarget.style.background=AM;e.currentTarget.style.transform='translateY(-1px)';e.currentTarget.style.boxShadow='0 10px 24px -8px rgba(4,120,87,0.45)';};
const nextLeave=e=>{e.currentTarget.style.background=canAdv?AD:'#CBD5E1';e.currentTarget.style.transform='translateY(0)';e.currentTarget.style.boxShadow=canAdv?'0 1px 2px rgba(15,23,42,0.08)':'none';};
const chip=(label,value)=>(<div className="ff-num flex-none text-center" style={{background:C.surfaceAlt,border:`1px solid ${BFAINT}`,borderRadius:9,padding:'5px 9px',minWidth:58}}><div className="ff-body" style={{fontSize:8,fontWeight:700,letterSpacing:'0.1em',color:C.faint}}>{label}</div><div style={{fontSize:11.5,color:C.ink2,fontWeight:600,marginTop:1}}>{value}</div></div>);
const Card=({active,onClick,title,blurb,right,icon,eyebrow})=>(<button onClick={onClick} className="text-left" style={{background:active?MINT:C.surface,border:`1px solid ${active?AD:BORDER}`,boxShadow:active?`0 0 0 3px ${ASOFT}`:'0 1px 2px rgba(15,23,42,0.03)',borderRadius:14,padding:'14px',cursor:'pointer',outline:'none',transition:'border-color 160ms, box-shadow 160ms'}} onMouseEnter={e=>{if(!active){e.currentTarget.style.borderColor='#CBD5E1';e.currentTarget.style.boxShadow='0 8px 20px -14px rgba(15,23,42,0.35)';}}} onMouseLeave={e=>{if(!active){e.currentTarget.style.borderColor=BORDER;e.currentTarget.style.boxShadow='0 1px 2px rgba(15,23,42,0.03)';}}}><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3 flex-1 min-w-0">{icon&&<span className="flex-none" style={{width:38,height:38,borderRadius:10,background:active?C.surface:ASOFT,border:active?`1px solid ${AB}33`:'none',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,lineHeight:1}}>{icon}</span>}<div className="flex-1 min-w-0">{eyebrow&&<div className="ff-body" style={{color:active?AD:C.muted,fontSize:9.5,fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase'}}>{eyebrow}</div>}<div className="ff-display" style={{color:C.ink,fontSize:15.5,fontWeight:700,lineHeight:1.2,letterSpacing:'-0.01em',marginTop:eyebrow?2:0}}>{title}</div>{blurb&&<div className="ff-body" style={{color:C.muted,fontSize:11.5,lineHeight:1.45,marginTop:3}}>{blurb}</div>}</div></div><div className="flex items-center gap-2 flex-none">{right}<span style={{width:20,height:20,borderRadius:'50%',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',background:active?AD:'transparent',border:active?'none':`1.5px solid ${BORDER}`,transition:'background 160ms'}}>{active&&<Check size={12} color="#fff"/>}</span></div></div></button>);
return(<div className="fixed inset-0 z-50 flex items-center justify-center anim-fade-in" style={{background:'rgba(15,23,42,0.55)',backdropFilter:'blur(4px)',padding:16}} onClick={allowSkip?onClose:undefined}><div onClick={e=>e.stopPropagation()} className="w-full" style={{maxWidth:760,background:C.surface,border:`1px solid ${BFAINT}`,borderRadius:20,overflow:'hidden',boxShadow:'0 40px 90px -28px rgba(15,23,42,0.45), 0 12px 32px -16px rgba(15,23,42,0.18)',maxHeight:'92vh',display:'flex',flexDirection:'column'}}>
<div className="flex-none" style={{padding:'24px 28px 20px',borderBottom:`1px solid ${BFAINT}`,background:C.surface}}><div className="flex items-start justify-between gap-3"><div className="flex-1 min-w-0"><div className="ff-body" style={{color:AD,fontSize:11,fontWeight:700,letterSpacing:'0.14em',textTransform:'uppercase'}}>{STEPS[step].ey}</div><h3 className="ff-display" style={{color:C.ink,fontSize:'clamp(24px,3.4vw,30px)',fontWeight:700,lineHeight:1.12,letterSpacing:'-0.025em',marginTop:6}}>{STEPS[step].title}</h3><p className="ff-body" style={{color:C.muted,fontSize:13.5,marginTop:6,lineHeight:1.5}}>{STEPS[step].sub}</p></div>{allowSkip&&<button onClick={onClose} aria-label="Close" className="flex-none" style={{color:C.faint,padding:6,borderRadius:8,background:'transparent',border:'none',cursor:'pointer',marginTop:2}}><X size={18}/></button>}</div><div className="mt-5" style={{display:'flex',alignItems:'center',gap:12}}><div style={{flex:1,height:6,borderRadius:999,background:C.bgWarm,overflow:'hidden'}}><div style={{height:'100%',borderRadius:999,width:`${((step+1)/STEPS.length)*100}%`,background:`linear-gradient(90deg,${AM},${AB})`,transition:'width 320ms cubic-bezier(0.16,1,0.3,1)'}}/></div><span className="ff-num" style={{fontSize:11,color:C.muted,fontWeight:600,flexShrink:0}}>{String(step+1).padStart(2,'0')}/{String(STEPS.length).padStart(2,'0')}</span></div></div>
<div className="overflow-y-auto flex-1" style={{padding:'22px 28px',minHeight:0}}>
{step===0&&(<div><input type="text" value={name} placeholder="e.g. Coffee Shop in Tel Aviv" onChange={e=>setName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&canAdv)next();}} onFocus={focusRing} onBlur={blurRing} autoFocus className="w-full ff-display outline-none" style={{padding:'16px 18px',borderRadius:14,fontSize:'clamp(20px,2.6vw,26px)',fontWeight:700,letterSpacing:'-0.02em',background:C.surface,border:`1px solid ${BORDER}`,color:C.ink,boxSizing:'border-box',transition:'border-color 160ms, box-shadow 160ms'}}/><div className="ff-body flex items-center gap-2" style={{color:C.muted,fontSize:12,marginTop:12}}><span>Press</span><kbd className="ff-num" style={{padding:'2px 7px',borderRadius:6,border:`1px solid ${BORDER}`,background:C.surfaceAlt,fontSize:10.5,color:C.ink2,fontWeight:600}}>Enter</kbd><span>to continue</span></div></div>)}
{step===1&&(<div><div className="flex items-center gap-2 mb-5 flex-wrap"><div className="relative flex-1" style={{minWidth:200}}><Search size={15} color={C.faint} style={{position:'absolute',left:13,top:'50%',transform:'translateY(-50%)',pointerEvents:'none'}}/><input type="text" value={search} onChange={e=>setSearch(e.target.value)} onFocus={focusRing} onBlur={blurRing} placeholder="Search 'coffee', 'app', 'gym'..." className="w-full ff-body outline-none" style={{padding:'11px 12px 11px 38px',borderRadius:12,border:`1px solid ${BORDER}`,background:C.surface,color:C.ink,fontSize:13.5,boxSizing:'border-box',transition:'border-color 160ms, box-shadow 160ms'}}/></div><button onClick={surp} className="ff-body flex items-center gap-2" style={{padding:'11px 16px',borderRadius:12,background:ASOFT,border:`1px solid ${AB}33`,color:AD,fontWeight:600,fontSize:13,cursor:'pointer',whiteSpace:'nowrap'}}><Wand2 size={15}/> Surprise me</button></div><div className="space-y-5">{BUSINESS_CATEGORIES.map(cat=>{const items=Object.entries(filtered).filter(([_,b])=>b.category===cat);if(!items.length)return null;return(<div key={cat}><div className="ff-body mb-2.5" style={{color:C.muted,fontSize:10,fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase'}}>{cat}</div><div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">{items.map(([key,biz])=>(<Card key={key} active={sK===key} onClick={()=>setSK(key)} icon={biz.icon} title={biz.label} blurb={biz.blurb} right={chip('NET MARGIN',`${biz.benchmarks.netMargin[0]}–${biz.benchmarks.netMargin[1]}%`)}/>))}</div></div>);})}{Object.keys(filtered).length===0&&<div className="text-center py-8"><div className="ff-body text-[13px]" style={{color:C.muted}}>No matches for "{search}".</div><button onClick={()=>{setSK('other');setSearch('');}} className="ff-body text-[12px] mt-3 inline-flex items-center gap-1.5" style={{background:C.surface,border:`1px solid ${BORDER}`,color:C.ink2,padding:'8px 14px',borderRadius:999,cursor:'pointer'}}>Use "Something Else" instead <ArrowRight size={13}/></button></div>}</div></div>)}
{step===2&&(<div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">{Object.entries(REGIONS).map(([key,reg])=>(<Card key={key} active={rK===key} onClick={()=>setRK(key)} title={reg.label} right={chip('TAX',`${reg.taxRate}%`)}/>))}</div>)}
{step===3&&(<div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">{Object.entries(CURRENCIES).map(([key,c])=>(<Card key={key} active={cK===key} onClick={()=>setCK(key)} icon={c.symbol} title={c.label} blurb={c.name}/>))}</div>)}
{step===4&&(<div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">{Object.entries(STATEMENT_OPTIONS).map(([key,s])=>(<Card key={key} active={stmts===key} onClick={()=>setStmts(key)} title={s.label} blurb={s.blurb}/>))}</div>)}
</div>
<div className="flex items-center justify-between flex-none gap-3" style={{padding:'16px 28px',borderTop:`1px solid ${BFAINT}`,background:C.surface}}><div className="flex items-center gap-1"><button onClick={()=>setStep(Math.max(0,step-1))} disabled={step===0} className="ff-body flex items-center gap-1.5" style={{padding:'9px 14px',borderRadius:999,color:step===0?C.faint:C.ink2,fontSize:13,fontWeight:500,background:'transparent',border:'none',opacity:step===0?0.5:1,cursor:step===0?'default':'pointer',pointerEvents:step===0?'none':'auto'}}><ArrowLeft size={15}/> Back</button>{onStartManual&&<button onClick={onStartManual} className="ff-body" style={{padding:'9px 12px',borderRadius:999,color:C.muted,fontSize:13,background:'transparent',border:'none',cursor:'pointer'}} title="Skip the wizard and start with an empty model (all values 0)">Start blank</button>}</div><div className="flex items-center gap-2.5">{!canAdv&&<button onClick={skipStep} className="ff-body" style={{padding:'9px 16px',borderRadius:999,color:C.ink2,fontSize:13,fontWeight:500,background:C.surface,border:`1px solid ${BORDER}`,cursor:'pointer'}}>Skip</button>}<button onClick={next} disabled={!canAdv} onMouseEnter={nextEnter} onMouseLeave={nextLeave} className="ff-body flex items-center gap-2" style={{padding:'11px 22px',borderRadius:999,background:canAdv?AD:'#CBD5E1',color:'#fff',fontSize:13.5,fontWeight:600,cursor:canAdv?'pointer':'not-allowed',border:'none',boxShadow:canAdv?'0 1px 2px rgba(15,23,42,0.08)':'none',transition:'background 160ms, box-shadow 160ms, transform 160ms'}}>{step===STEPS.length-1?'Build my model':'Continue'} <ArrowRight size={15}/></button></div></div>
</div></div>);
}
