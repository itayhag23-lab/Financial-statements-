...
const STATEMENT_OPTIONS={incomeOnly:{label:'Income only',blurb:'Revenue, costs, profit. The fastest start.'},incomeAndCF:{label:'Income + Cash Flow',blurb:'Adds a cash view so you can track runway.'},full:{label:'The full picture',blurb:'Income, Balance Sheet and Cash Flow. They link automatically.'}};
...
<div className="flex items-center gap-2 ff-body text-[10.5px]" style={{color:C.muted}}><span className="label-eyebrow">Sections</span><span>·</span><span>{eP} of {tP} open</span></div>
...
<div className="flex-1 min-w-[200px]"><div className="label-eyebrow ff-body inline-flex items-center gap-0.5" style={{color:C.gold}}>Feasibility<HelpTooltip term="Feasibility score" what="A 0 to 100 health check that blends five signals from your Base scenario: profitability, margins, growth realism, cash and balance." why="A quick sense of whether the plan hangs together. Treat it as a signal to improve, not a grade or a guarantee." size={12}/></div><div className="ff-display text-[28px] mt-0.5" style={{color:tone,fontWeight:500}}>{label}</div><div className="ff-body text-[12px] mt-2" style={{color:C.ink2}}>Composite score across five dimensions. Computed from Base scenario.</div>
...
useEffect(()=>{if(!moreOpen)return;const onDoc=(e)=>{if(moreRef.current&&!moreRef.current.contains(e.target))setMoreOpen(false);};const onKey=(e)=>{if(e.key==='Escape')setMoreOpen(false);};document.addEventListener('mousedown',onDoc);document.addEventListener('keydown',onKey);return()=>{document.removeEventListener('mousedown',onDoc);document.removeEventListener('keydown',onKey);};},[moreOpen]);
...
const STEPS=[{id:'name',ey:'Setup · 1 of 3',title:'Name your model',sub:'A rough name is perfect. You can rename it anytime.'},{id:'business',ey:'Setup · 2 of 3',title:'What are you modeling?',sub:'This seeds realistic revenue, costs and margins to start from.'},{id:'currency',ey:'Setup · 3 of 3',title:'What currency?',sub:'Every figure shows in this. Pick your reporting currency.'}];
const canAdv=step===0?name.trim().length>0:step===1?!!sK:!!cK;
// Start every model with just the Income Statement (income-only) at a default
// tax region; users add the Balance Sheet / Cash Flow and adjust tax later.
const withDefaults=()=>({name:name.trim()||'Untitled Project',sectorKey:sK||'other',regionKey:'us',currencyKey:cK||'usd',incomeType:'main',stage:'early',statements:'incomeOnly'});
...
{target:'[data-tour="model-table"]',placement:'top',onBeforeStep:()=>setBuildTab('income'),title:'This is your model',body:'Every figure below is editable. Click any cell in a year column, type a number, and the whole statement recalculates instantly.'},
{target:'[data-tour="row-help"]',placement:'bottom',onBeforeStep:()=>setBuildTab('income'),title:'Not sure what a line means?',body:'Every line has a small question mark icon next to its name. Click one to learn what that line is, why it matters, and where its number comes from.'},
{target:'[data-tour="scenario"]',placement:'bottom',title:'Compare Base, Best and Worst',body:'Switch scenarios here. Edits apply only to the selected scenario, so you can build a realistic Base, then stress test the upside and downside.'},
...
{target:'[data-tour="analysis"]',placement:'left',title:'Score, analyze and export',body:'The badge is your feasibility score. Open Analysis for what-if and sensitivity, and use Excel or Share in the toolbar to export.'},
{target:'[data-tour="take-tour"]',placement:'bottom',title:'Replay anytime',body:'That’s the tour. Reopen it from here whenever you like. Next up, a 30 second intro to what your three statements actually mean.'},
],[]);
